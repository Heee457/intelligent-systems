...
test.describe('authentication and role routing', () => {
  ...
  test('shows a useful error when login credentials are invalid', async ({ page }) => {
    ...
    await page.getByPlaceholder('请输入用户名').fill(uniqueUsername('student', 'Missing Login User'))
  ...
  test('allows a registered teacher to log in from a fresh browser session', async ({ browser }) => {
    ...
    const teacher = await registerUser(setupPage, 'teacher', { email, password: testPassword, name: 'E2E Login Teacher' })
    await setupPage.close()

    const page = await browser.newPage()
    await page.goto('/login')
    await page.getByPlaceholder('请输入用户名').fill(teacher.name)
